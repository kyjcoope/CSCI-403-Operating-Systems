...
public class MemManager{
    ...
    int makeFreeFrame() 
    {
      ...
      if(policy == FIFO){
	    ...
	    victim = (victim + 1) % Machine.NumPhysPages;
         ...
         int least = (1<<hbits) -1; //sets least bits based on hbit to give max reference bits
         for(i = 0; i < Machine.NumPhysPages; i++){ //loop through
            if(history[i] < least){ //endif
               least = history[i]; //new least
               victim = i; //new victim
            }
         } //endfor
      } //end elseif
         ...
         for(i = 0;i< Machine.NumPhysPages; i++){
            if(pick > 0 && !coreOwners[i].use && !coreOwners[i].dirty){
               victim = i; //new victim
               pick = 0;
            }//end if
            if(pick > 1 && !coreOwners[i].use && coreOwners[i].dirty){
               victim = i; //new victim
               pick = 1;
            }//end if
            if(pick > 2 && coreOwners[i].use && !coreOwners[i].dirty){
               victim = i; //new victim
               pick = 2;
            } //end if
            if(pick > 3 && coreOwners[i].use && coreOwners[i].dirty){
               victim = i; //new victim
               pick = 3;
            } //end if
         }//end for
         for(i = 0; i<Machine.NumPhysPages;i++){
            if(coreOwners[i] != null && coreOwners[i].legal){
    ...
    void recordHistory( int arg )
    {
     ...
     if(policy == LRU){
	   ...
	   history[i] = history[i] >> 1; //shift reference bits
           if(coreOwners[i] != null && coreOwners[i].legal){ //check if legal
	      if(coreOwners[i].use){ //if was used
	         history[i] = history[i]|(1 << (hbits - 1)); //sets high bit of reference bits
                 coreOwners[i].use = false; //reset use flag
              } //endif
           } //endif
        } //endfor
     } //endif
